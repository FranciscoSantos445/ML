...
def grid_search_arx(y, u):
    ...
    return best_n, best_m, best_d, best_model, y_best_pred, y_best_test, phi_out
...
n,m,d,model,y_pred,y_test_out,phi = grid_search_arx(y, u)
...
plt.scatter(range(len(phi)), phi, color='green', label='Phi')
plt.scatter(range(len(y_test_out)), y_test_out, color='blue', label='Generated Output')
plt.scatter(range(len(y_pred)), y_pred, color='red', label='Generated Output')
